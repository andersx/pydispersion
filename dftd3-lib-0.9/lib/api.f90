...
module dftd3_api
  ...
  public :: dftd3_dispersion, dftd3_pbc_dispersion, get_c6, get_c6_pbc
...
contains
  ...
  !! \param grads  Calculated gradients in atomic units, if present.
  !!
  subroutine get_c6(this, coords, izp, disp, grads, c6mat)
    type(dftd3_calc), intent(in) :: this
    real(wp), intent(in) :: coords(:,:)
    integer, intent(in) :: izp(:)
    real(wp), intent(out) :: disp
    real(wp), optional, intent(out) :: grads(:,:)
    real(wp), intent(out) :: c6mat(:,:)
    ...
    alp8 = alp6 + 2.0_wp
    alp10 = alp8 + 2.0_wp
    ! write (*,*) "Print BEGIN GETTING C6"

    call c6interface(max_elem, maxc, natom, coords, izp, this%c6ab, this%mxc, &
        & r2r4, this%r0ab, rcov, rs6, rs8, rs10, alp6, alp8, alp10, &
        & this%version, this%noabc, this%rthr, this%cn_thr, e6, e8, e10, e12, &
        & e6abc, c6mat)
   
    ! write (*,*) "Print END GETTING C6"
  ...
  !! \param stress  Calculated stress tensor in atomic units, if present.
  !!
  subroutine get_c6_pbc(this, coords, izp, latvecs, disp, grads, &
      & stress, c6mat)
    type(dftd3_calc), intent(in) :: this
    real(wp), intent(in) :: coords(:,:)
    integer, intent(in) :: izp(:)
    ...
    call pbc_c6interface(max_elem, maxc, natom, coords, izp, this%c6ab, this%mxc, &
        & r2r4, this%r0ab, rcov, rs6, rs8, rs10, alp6, alp8, alp10, &
        & this%version, this%noabc, e6, e8, e10, e12, e6abc, latvecs, &
        & this%rthr, rep_vdw, this%cn_thr, rep_cn, c6mat)
    
  end subroutine get_c6_pbc
